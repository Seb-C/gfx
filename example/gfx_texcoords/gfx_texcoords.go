...
// directory:
//  $GOPATH/src/azul3d.org/gfx.v2/example
...
func absPath(relPath string) string {
	if len(examplesDir) == 0 {
		...
		for _, path := range filepath.SplitList(build.Default.GOPATH) {
			path = filepath.Join(path, "src/azul3d.org/gfx.v2/example")
...
func gfxLoop(w window.Window, r gfx.Renderer) {
	...
	// Load the picture.
	f, err := os.Open(absPath("gfx_texcoords/texture_coords_1024x1024.png"))
	if err != nil {
		log.Fatal(err)
	}
